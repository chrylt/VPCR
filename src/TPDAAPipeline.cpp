...
void TPDAAPipeline::CreateBatchList()
{
...
void TPDAAPipeline::CreateRenderTarget()
{
    const auto res = config_.Get<std::vector<std::uint32_t>>("resolution").value();
    for (auto& pipeline : pipelines_) {
        const tga::BufferInfo info{tga::BufferUsage::storage,
                                   static_cast<size_t>(res[0] * res[1]) * sizeof(HistogramTP)};
        pipeline.renderTarget = backend_.createBuffer(info);
    }
}

void TPDAAPipeline::CreateDepthBuffer()
{
    const auto res = config_.Get<std::vector<std::uint32_t>>("resolution").value();
    for (auto& pipeline : pipelines_) {
...
void TPDAAPipeline::CreateClearPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void TPDAAPipeline::CreateLODPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void TPDAAPipeline::CreateProjectionDepthPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void TPDAAPipeline::CreateProjectionColorPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void TPDAAPipeline::CreateDisplayPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
