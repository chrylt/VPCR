...
void OPDAAPipeline::Execute(const std::uint32_t frameIndex, const std::span<const UploadData *const> uploads,
...
void OPDAAPipeline::CreateBatchList()
{
...
void OPDAAPipeline::CreateDepthBuffer()
{
    const auto res = config_.Get<std::vector<std::uint32_t>>("resolution").value();
    for (auto& pipeline : pipelines_) {
        const tga::BufferInfo info{tga::BufferUsage::storage, static_cast<size_t>(res[0] * res[1]) * sizeof(Histogram)};
        pipeline.depthBuffer = backend_.createBuffer(info);
    }
}

void OPDAAPipeline::CreateClearPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void OPDAAPipeline::CreateLODPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void OPDAAPipeline::CreateProjectionPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
...
void OPDAAPipeline::CreateDisplayPass(const Resources resources)
{
    for (auto& pipeline : pipelines_) {
